...
A taxonomy could be disabled with `enabled: false`.

### `menus`

Each menu should have the following properties:
...
A menu entry can be overridden: use the same index `id`.
...
Default pages, created from built-in templates:
...
List of virtual pages (with a front matter only).
...
Array of generated output formats for each page type.
...
- `exclude`: list of excluded files from copy
...
Localized configuration variables must be stored under `config` key.
