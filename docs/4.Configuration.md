...
The local website configuration file (`config.yml`) overrides the [Cecil default configuration](https://github.com/Cecilapp/Cecil/blob/master/config/default.php#L11) (`default.php`).
...
- `fingerprint`: Adds the file fingerprint in the filename (ie: `style.c9a87d1be1c3cf7c3837c968b28364ec.min.css`)
- `compile`: Compiles a SCSS into a CSS
- `minify`: Compresses the file content (Available for file with a `text/css` and `text/javascript` MIME Type)
