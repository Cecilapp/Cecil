...
| `section`        | Section                 | Page's _Section_ (ie: `blog`). Can’t be overridden.          |
...
> See [_Configuration_](4.Configuration.md#menus) for details.
...
> `exclude` is different from [`published`](#predefined): an excluded page is published but excluded from section pages' list.
