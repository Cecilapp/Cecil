...
1. [**Markdown**](https://daringfireball.net/projects/markdown/) (or plain text) files
2. **data** collections files
3. **static** files (assets, medias, etc.)
...
> 2. Content of files in `data/` are exposed in [templates](3.Templates.md#site) with `{{ site.data }}`
> 3. Files in `static/` are copied as is in the root of the built website (ie: `static/logo.png` -> `logo.png`)
...
#### Image resize (experimental)

Images files can be resized with the following syntax: `?resize=300`.

**Notes :**

- The original file is not altered and the resized version is stored in `/images/thumbs/<resize>/path/to/image.ext`
- It required [GD extension](https://www.php.net/manual/fr/book.image.php), otherwise it « resizes » images only with the `width` HTML attribute
...
## Localized content (experimental)
...
## Variables and macros (experimental)

> Variables and macros are experimetal features.
...
2 macros [are available](https://github.com/Cecilapp/Cecil/blob/master/res/layouts/macros.twig):
