...
Creates an asset (CSS, JavaScript, media file) from a file path or an array of files path (bundle).
...
| filename    | File to save a bundle.                              | string  |         |
...
:   A list template with pagination used by sections.
...
`robots.txt.twig`
:   A `robots.txt` template: allow all pages except 404, with a link to the XML sitemap.

`sitemap.xml.twig`
:   A `sitemap.xml` template: list amm pages sorted by date.
