...
Turns a Page, an Asset or a string in an URL.
...
You must use `url` filter to publish (save) an asset:
```twig
{{ asset('style.css')|minify|url }}
```
...
### `url(Page|Asset|page-id|path|URL, options)`

See [`url` filter](#url-options).
...
- `minify`: `true|false`
- `version`: `true|false`
- `attributes`: HTML elements attributes

```twig
{{ asset('image.png', {attributs:{title:'title',alt:'alternative'}})|html }}
```
