...
| ----------------- | ------------------------------------------------------------ |
| `site.pages`      | Collection of published pages (in the current language).     |
| `site.pagesintl`  | Collection of published pages (in all languages).            |
...
| lang      | Defines the required language.           | string  | Main language |
...
{{ page|url({format:json,lang:site.language}) }}
```
...
[*Gettext PHP extension*](https://www.php.net/gettext) is required.  
...
[*Intl PHP extension*](https://www.php.net/manual/book.intl.php) is required.  
